        ...
        try:
            ...
            if prediction_repo.prediction_exists_for_date(next_day):
                ...
                try:
                    ...
                    logger.info(f"✅ Generated and saved prediction for {next_day}: ${predicted_price:.2f} (Method: {prediction_method})")
...
async def get_prediction_reasons(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"🤖 No saved reasons found for {next_day}. Generating on-demand...")
            ...
            try:
                logger.info(f"🤖 Starting prediction reason generation for {next_day}")
                ...
                if prediction_reasons:
                    ...
                    try:
                        ...
                        logger.info(f"✅ Generated and saved prediction reasons for {next_day}")
                    except Exception as save_error:
                        logger.warning(f"⚠️ Failed to save prediction reasons to database: {save_error}")
                    ...
                    logger.warning(f"⚠️ Failed to generate prediction reasons for {next_day} - Gemini returned None")
                    return {
                        "status": "error",
