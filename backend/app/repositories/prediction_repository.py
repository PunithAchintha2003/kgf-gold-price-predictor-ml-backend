...
class PredictionRepository:
    ...
    def get_accuracy_stats() -> Dict:
        ...
        # Reuse comprehensive stats so totals, evaluated, and averages stay consistent
        try:
            comprehensive = PredictionRepository.get_comprehensive_stats()
            
            result = {
                'average_accuracy': comprehensive.get('average_accuracy', 0.0),
                'r2_score': comprehensive.get('r2_score'),
                'total_predictions': comprehensive.get('total_predictions', 0),
                'evaluated_predictions': comprehensive.get('evaluated_predictions', 0)
            }
            ...
            if "no such table" in error_msg or "does not exist" in error_msg or "relation" not in error_msg:
                logger.debug(f"Predictions table not available when getting accuracy stats: {e}")
            else:
                logger.warning(f"Error getting accuracy stats from comprehensive stats: {e}")
