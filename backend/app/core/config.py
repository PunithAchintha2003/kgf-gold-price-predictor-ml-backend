...
class Settings:
    ...
    def __init__(self):
        ...
        self.auto_update_enabled: bool = os.getenv("AUTO_UPDATE_ENABLED", "true").lower() == "true"
        self.auto_update_interval: int = int(os.getenv("AUTO_UPDATE_INTERVAL", "3600"))  # Default: 1 hour
        self.auto_update_startup_delay: int = int(os.getenv("AUTO_UPDATE_STARTUP_DELAY", "60"))  # Default: 1 minute
        self.auto_update_max_retries: int = int(os.getenv("AUTO_UPDATE_MAX_RETRIES", "3"))
        self.auto_update_retry_delay: int = int(os.getenv("AUTO_UPDATE_RETRY_DELAY", "300"))  # 5 minutes
    ...
    def _validate(self):
        ...
        if self.cache_duration < 0:
            errors.append(f"cache_duration must be non-negative, got {self.cache_duration}")
        
        # Validate PostgreSQL settings if using PostgreSQL
        if self.use_postgresql:
            if not self.postgresql_password:
                errors.append("POSTGRESQL_PASSWORD is required when USE_POSTGRESQL=true")
        ...
        if self.max_request_size < 1024:  # At least 1KB
            errors.append(f"MAX_REQUEST_SIZE must be at least 1024 bytes, got {self.max_request_size}")
        
        # Validate background task settings
        if self.auto_update_interval < 60:  # At least 1 minute
            errors.append(f"AUTO_UPDATE_INTERVAL must be at least 60 seconds, got {self.auto_update_interval}")
        if self.auto_update_startup_delay < 0:
            errors.append(f"AUTO_UPDATE_STARTUP_DELAY must be non-negative, got {self.auto_update_startup_delay}")
        if self.auto_update_max_retries < 0:
            errors.append(f"AUTO_UPDATE_MAX_RETRIES must be non-negative, got {self.auto_update_max_retries}")
        
        # Warn about production settings
        if self.environment == "production":
            if "*" in self.cors_origins:
                errors.append("CORS_ORIGINS should not be '*' in production. Set specific origins.")
