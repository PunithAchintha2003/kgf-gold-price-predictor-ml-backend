        ...
        try:
            ...
            if self.news_enhanced_predictor and self.news_enhanced_predictor.model is not None:
                try:
                    ...
                    sentiment_features = self.news_enhanced_predictor.fetch_and_analyze_news(days_back=7)
                    ...
                    prediction = self.news_enhanced_predictor.predict_with_news(enhanced_features)
            ...
            if self.lasso_predictor and self.lasso_predictor.model is not None:
                try:
                    ...
                    # Create features
                    features_df = self.lasso_predictor.create_fundamental_features(market_data)
                    
                    if features_df.empty:
                        raise ValueError("No features created")
                    
                    # Make prediction using Lasso Regression
                    prediction = self.lasso_predictor.predict_next_price(features_df)
