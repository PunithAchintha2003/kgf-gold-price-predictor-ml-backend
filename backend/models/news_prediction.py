...
class NewsSentimentAnalyzer:
    ...
    def _fetch_alpha_vantage_data(self, days_back: int) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Fetched {len(news_items)} articles from Alpha Vantage")
            return news_items
    ...
    def _fetch_yahoo_news_data(self, days_back: int) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Fetched {len(news_items)} articles from Yahoo Finance")
            return news_items
    ...
    def create_sentiment_features(self, sentiment_df: pd.DataFrame) -> pd.DataFrame:
        ...
        daily_sentiment['sentiment_trend'] = daily_sentiment['combined_sentiment_mean'].rolling(5).mean()
        daily_sentiment['sentiment_momentum'] = daily_sentiment['combined_sentiment_mean'].diff()
        
        # Create lagged features
        for lag in [1, 2, 3, 5, 10]:
            daily_sentiment[f'sentiment_lag_{lag}'] = daily_sentiment['combined_sentiment_mean'].shift(lag)
            daily_sentiment[f'news_volume_lag_{lag}'] = daily_sentiment['news_count'].shift(lag)
        
        # Rolling statistics
        for window in [5, 10, 20]:
            daily_sentiment[f'sentiment_ma_{window}'] = daily_sentiment['combined_sentiment_mean'].rolling(window).mean()
            daily_sentiment[f'sentiment_std_{window}'] = daily_sentiment['combined_sentiment_mean'].rolling(window).std()
            daily_sentiment[f'news_volume_ma_{window}'] = daily_sentiment['news_count'].rolling(window).mean()
...
class NewsEnhancedLassoPredictor:
    ...
    def fetch_and_analyze_news(self, days_back: int = 30) -> pd.DataFrame:
        ...
        """
        logger.info(f"Fetching and analyzing news data for the last {days_back} days")
        ...
        self.sentiment_features = self.news_analyzer.create_sentiment_features(sentiment_df)
        
        logger.info(f"Created sentiment features with shape: {self.sentiment_features.shape}")
    ...
    def create_enhanced_features(self, market_data: Dict, sentiment_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        if sentiment_df is not None and not sentiment_df.empty:
            ...
            sentiment_cols = [col for col in enhanced_features.columns if 'sentiment' in col or 'news' in col]
            enhanced_features[sentiment_cols] = enhanced_features[sentiment_cols].fillna(0)
            
            # Remove date column
            enhanced_features = enhanced_features.drop('date', axis=1)
            
            logger.info(f"Enhanced features created with shape: {enhanced_features.shape}")
            logger.info(f"Added {len(sentiment_cols)} sentiment features")
            
            return enhanced_features
        else:
            logger.warning("No sentiment data available, using base features only")
    ...
    def train_enhanced_model(self, enhanced_features: pd.DataFrame, target_col='gold_close', test_size=0.2):
        ...
        """
        logger.info("Training enhanced Lasso model with news sentiment features")
        ...
        feature_cols = [col for col in data_clean.columns if col not in ['target', target_col]]
        ...
        logger.info(f"Selected {len(self.selected_features)} features out of {len(self.feature_columns)}")
    ...
    def predict_with_news(self, enhanced_features: pd.DataFrame) -> float:
        ...
        available_features = [col for col in self.feature_columns if col in enhanced_features.columns]
    ...
    def load_enhanced_model(self, filepath='enhanced_lasso_gold_model.pkl'):
        ...
        self.news_analyzer = model_data.get('news_analyzer', NewsSentimentAnalyzer())
...
def main():
    ...
    enhanced_features = predictor.create_enhanced_features(market_data, sentiment_features)
